  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-50 to-emerald-50">
    ...
    <div class="max-w-6xl mx-auto p-6">
      ...
      <div class="text-center mb-8">
        ...
        <h1 class="text-3xl font-bold text-gray-800 mb-2">璟的文章</h1>
        <p class="text-gray-600">欣賞璟的創作世界</p>
      </div>
      ...
      <div v-else-if="articles.length > 0" class="grid gap-6">
        ...
        >
          ...
          <div class="bg-gradient-to-r from-emerald-600 to-teal-600 p-6">
            <h2 class="text-xl font-bold text-white mb-2">{{ article.title }}</h2>              <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4 text-emerald-100 text-sm">
                ...
                <div class="flex items-center space-x-2">
                  ...
                  <button
                    ...
                    :class="isFavorited(article.id) ? 'text-yellow-300' : 'text-emerald-100'"
          ...
          <div class="p-6">
            ...
            <div v-if="article.tags && article.tags.length > 0" class="mt-4 flex flex-wrap gap-2">
              <span
                ...
                :key="tag"
                class="inline-block bg-emerald-100 text-emerald-800 px-3 py-1 rounded-full text-sm"
              >
                #{{ tag }}
              </span>
            </div>
      ...
      <div v-else class="text-center py-12">
        ...
        <p class="text-gray-500">璟還沒有發布任何文章</p>
      </div>
    </div>
  </div>
</template>
    ...
    if (process.client) {
      ...
      if (isCurrentlyFavorited) {
        ...
        for (const docSnapshot of querySnapshot.docs) {
          await deleteDoc(doc(db, 'qian_favorites', docSnapshot.id))
        }
        ...
        // 添加收藏
        const favoriteRef = doc(collection(db, 'qian_favorites'))
...
// 設置頁面標題
useHead({
  title: '觀賞空間 - 璟的文章 | WriteApp',
  meta: [
    { name: 'description', content: '欣賞璟的創作作品' }
  ]
})
</script>
