  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-50 to-pink-50">
    ...
    <div class="max-w-5xl mx-auto p-6">
        ...
        <div class="flex flex-wrap items-center gap-4 justify-between">
          ...
          <div v-if="userArticles.length > 0" class="flex flex-wrap gap-2">
            ...
            <select 
              ...
              class="border border-gray-300 rounded-lg px-3 py-1 text-sm focus:outline-none focus:border-pink-500"
            >
      ...
      <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
        <!-- 文章標題輸入 -->
        <div class="bg-pink-700 p-6">
          <input
            v-model="article.title"
            type="text"
            placeholder="請輸入文章標題..."
            class="w-full bg-white/10 border border-pink-500/50 rounded-xl px-4 py-3 text-white text-xl placeholder-pink-200 focus:outline-none focus:border-pink-300 focus:ring-2 focus:ring-pink-300/50"
          />
        </div>

        <!-- 寫作工具欄 -->
        <div class="bg-pink-600 px-6 py-3 flex items-center justify-between flex-wrap gap-2">
          <div class="flex items-center space-x-4 flex-wrap gap-2">
            <span class="text-white text-sm">字數: {{ wordCount }}</span>
            <span class="text-pink-200 text-sm">|</span>
        ...
        <div class="bg-gray-50 p-6 border-t">
          <div class="flex flex-wrap items-center gap-4">
            <div class="flex items-center space-x-2">
              ...
              <button
                @click="addTag"
                class="bg-pink-600 hover:bg-pink-700 text-white px-3 py-1 rounded-lg text-sm transition-colors"
              >
                新增
              </button>
            </div>
          ...
          <div class="mt-3 flex flex-wrap gap-2">
            <span
              ...
              :key="index"
              class="inline-flex items-center bg-pink-100 text-pink-800 px-3 py-1 rounded-full text-sm"
            >
              {{ tag }}
              <button
      ...
      <div v-if="statusMessage" class="mt-4 text-center">
        <div class="inline-block bg-pink-100 text-pink-800 px-4 py-2 rounded-lg">
          {{ statusMessage }}
        </div>
      </div>
    </div>
